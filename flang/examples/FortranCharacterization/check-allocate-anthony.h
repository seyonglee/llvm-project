...
public:
  ...
  bool checkScalar();
  bool checkAllocatableCharacter();
...
bool AllocationCheckerHelper::checkScalar() {
  if (rank_ == 0)
    if (type_)
      if (type_->category() != DeclTypeSpec::Category::Character)
        return true;
...
bool AllocationCheckerHelper::checkAllocatableCharacter() {
  if (type_)
    if (type_->category() == DeclTypeSpec::Category::Character)
      if (ultimate_)
        if (IsAllocatable(*ultimate_))
          return true;
  return false;
}
