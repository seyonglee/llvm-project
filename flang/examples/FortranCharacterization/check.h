...
public:
  ...
  // Anthony features
  bool getTypeBoundProcPresent() { return typeBoundProcPresent; };
  bool getOverridePresent() { return overridePresent; }
...
private:
  ...
  bool CheckProcBindingForOverride(const Symbol &, const ProcBindingDetails &);
  ...
  // Anthony features
  bool typeBoundProcPresent{false};
...
void CheckHelper::Check(const Symbol &symbol) {
  ...
  bool isDone{false};
  // llvm::outs() << "symbolname: " << symbol.name() << "\n";
  common::visit(common::visitors{
                    [&](const UseDetails &x) {
                      // llvm::outs() << "\tUseDetails\n";
                      isDone = true;
                    },
                    [&](const HostAssocDetails &x) {
                      // llvm::outs() << "\tHostAssocDetails\n";
                      CheckHostAssoc(symbol, x);
                      isDone = true;
                    },
                    [&](const ProcBindingDetails &x) {
                      llvm::outs() << symbol.name() << "\n";
                      typeBoundProcPresent = true;
                      // llvm::outs() << "\tProcBindingDetails\n";
                      CheckProcBinding(symbol, x);
                      overridePresent = CheckProcBindingForOverride(symbol, x);
                      isDone = true;
                    },
                    [&](const ObjectEntityDetails &x) {
                      // llvm::outs() << "\tObjectEntityDetails\n";
                      CheckObjectEntity(symbol, x);
                    },
                    [&](const ProcEntityDetails &x) {
                      // llvm::outs() << "\tProcEntityDetails\n";
                      CheckProcEntity(symbol, x);
                    },
                    [&](const SubprogramDetails &x) {
                      // llvm::outs() << "\tSubprogramDetails\n";
                      CheckSubprogram(symbol, x);
                    },
                    [&](const DerivedTypeDetails &x) {
                      // llvm::outs() << "\tDerivedTypeDetails\n";
                      CheckDerivedType(symbol, x);
                    },
                    [&](const GenericDetails &x) {
                      // llvm::outs() << "\tGenericDetails\n";
...
bool CheckHelper::CheckProcBindingForOverride(
  ...
  // llvm::outs() << "\t" << symbol.name() << ": made it to checkprocbinding\n";
  // llvm::outs() << "\t" << binding.symbol() << "<- symbol is bound to this\n";
  ...
  if (const Symbol *
    ...
    if (const auto *overriddenBinding{
      ...
      // llvm::outs() << "\t\tfound an overridden binding!\n";
    } else {
      SayWithDeclaration(*overridden,
...
void CheckHelper::Check(const Scope &scope) {
  ...
  if (scope.IsParameterizedDerivedTypeInstantiation()) {
    ...
    int mainProgCnt{0};
    // llvm::outs() << "start of scope children\n";
    ...
    // llvm::outs() << "end of scope children\n";
